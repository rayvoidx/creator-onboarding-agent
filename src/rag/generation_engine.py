    ...
    def _initialize_models(self):
        ...
        try:
            ...
            if self.openai_api_key:
                for model_name in [self.default_model, self.fast_model, self.fallback_model]:
                    if isinstance(model_name, str) and model_name.startswith('gpt-') and model_name not in self.models:
            ...
            if self.anthropic_api_key:
                for model_name in [self.default_model, self.deep_model]:
                    if isinstance(model_name, str) and model_name.startswith('claude') and model_name not in self.models:
            ...
            if self.google_api_key and ChatGoogleGenerativeAI:
                for model_name in [self.fast_model, self.fallback_model, self.default_model]:
                     if isinstance(model_name, str) and 'gemini' in model_name.lower() and model_name not in self.models:
    ...
    def _select_model(self, model_name: Optional[str], context: Optional[Dict[str, Any]]) -> tuple[Optional[str], Optional[Any]]:
        ...
        try:
            ...
            if context:
                ...
                if task in ['analysis', 'coding', 'reasoning'] and self.deep_model in self.models:
