...
class RAGPipeline:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        preferred_llms = self.config.get("llm_models") or self.config.get(
            "llm_candidates"
        )
        if preferred_llms:
        ...
        try:
            ...
            if self.langfuse:
                trace_id = await self.langfuse.start_trace(
                    name="rag_query",
                    user_id=(
                        (user_context or {}).get("user_id") if user_context else None
                    ),
                    session_id=(
                        (user_context or {}).get("session_id") if user_context else None
                    ),
            ...
            retrieved_docs = await self.retrieval_engine.rerank_documents(
                query, all_docs, top_k=self.rerank_top_k
            )

            ctx_task = asyncio.create_task(
                self._create_context(retrieved_docs, user_context)
            )
            ...
            complexity = (
                "high"
                if len(retrieved_docs) > 3 or len(processed_query) > 50
                else "low"
            )
            ...
            optimized_prompt, _ = self.prompt_optimizer.optimize(
                prompt, []
            )  # Prompt string optimization
            ...
            response = await self._generate_response(
                optimized_prompt, query_type, context
            )
        ...
        try:
            ...
            if self.enable_hybrid_search:
                keyword_results = await self.retrieval_engine.keyword_search(
                    query=query, limit=self.max_retrieval_docs
                )
    ...
    async def _create_context(
        self, documents: List[Dict[str, Any]], user_context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """컨텍스트 생성"""
        try:
            context = {
            ...
            # 문서 요약 생성
            if documents:
                context["document_summary"] = await self._create_document_summary(
                    documents
                )
            ...
            if spec:
                try:
                    enrichment = await self.mcp_service.enrich_context(
                        spec, user_context or {}
                    )
                    if enrichment:
                        context["mcp_enrichment"] = enrichment
                        if enrichment.get("external_snippets"):
                            context["external_snippets"] = enrichment[
                                "external_snippets"
                            ]
    ...
    async def _generate_response(
        self, prompt: str, query_type: PromptType, context: Dict[str, Any]
    ) -> str:
        """응답 생성"""
        try:
            # 시스템 프롬프트 선택
    ...
    async def _postprocess_response(
        ...
        try:
            ...
            final_response = await self.response_refiner.refine(
                ...
                context={
                    "retrieved_documents": documents
                },  # Pass docs for hallucination check
                style="wrtn_friendly",
