...
class HttpMCP:
    ...
    def __init__(
        ...
        if verify_ssl is None:
            verify_ssl = os.getenv("MCP_SSL_VERIFY", "false").lower() in (
                "true",
                "1",
                "yes",
            )
    ...
    def fetch(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # SSL 오류 발생 시 verify=False로 재시도
                logger.debug(
                    f"SSL error for {url}, retrying with verify=False: {ssl_err}"
                )
...
class WebSearchMCP:
    ...
    def _search_brave(self, query: str, top_k: int) -> List[str]:
        ...
        try:
            ...
            logger.warning(
                f"Government site search failed, falling back to general search: {e}"
            )
...
class YouTubeMCP:
    ...
    def __init__(
        ...
        if enable_analyzer:
            try:
                ...
                logger.debug(
                    "YouTubeMCP: YouTube Analyzer not available (yt-dlp required)"
                )
                self.analyzer = None
    ...
    def resolve_channel_id(
        ...
        """Resolve a channel identifier from various inputs."""
        if channel_id:
            return channel_id
        if channel_handle:
            handle = (
                ...
                else f"@{channel_handle}"
            )
            resolved = self._get_channel_id_by_handle(handle)
            if resolved:
                return resolved
        if channel_username:
            resolved = self._get_channel_id_by_username(channel_username)
            if resolved:
                return resolved
        return ""
    ...
    def get_channel_overview(
        ...
        """Return channel stats plus a snapshot of recent videos."""
        resolved_channel = self.resolve_channel_id(
            channel_id, channel_username, channel_handle
        )
        if not resolved_channel:
            return {}

        channel_info = self._get_channel_info(resolved_channel)
    ...
    def get_channel_videos(
        ...
        try:
            ...
            if not channel_info:
                logger.error(
                    f"YouTubeMCP: Failed to get channel info for: {channel_id}"
                )
                return []
            ...
            if not playlist_id:
                logger.error(
                    f"YouTubeMCP: No uploads playlist found for channel: {channel_id}"
                )
                return []
            ...
            logger.info(
                f"YouTubeMCP: Retrieved {len(videos)} videos from channel: {channel_id}"
            )
            return videos

        except Exception as e:
            logger.error(
                f"YouTubeMCP: Failed to get channel videos: {e}", exc_info=True
            )
            return []
    ...
    def _get_channel_id_by_handle(self, handle: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"YouTubeMCP: forHandle failed, trying search API: {handle_error}"
                )
    ...
    def _get_channel_id_by_username(self, username: str) -> str:
        ...
        try:
            ...
            logger.debug(
                f"YouTubeMCP: Failed to get channel ID by username (legacy): {e}"
            )
            return ""
    ...
    def _get_playlist_videos(
        ...
        try:
            ...
            logger.error(
                f"YouTubeMCP: Failed to get playlist videos: {e}", exc_info=True
            )
            return []
    ...
    def get_video_details_by_ids(
        self, video_ids: List[str]
    ) -> Dict[str, Dict[str, Any]]:
        """Public wrapper for video detail retrieval."""
    ...
    async def fetch_for_rag(
            ...
            try:
                ...
                if videos:
                    ...
                    try:
                        ...
                        logger.error(
                            f"YouTubeMCP: Failed to get latest videos: {e2}",
                            exc_info=True,
                        )
            except Exception as e:
        ...
        has_youtube_keyword = any(
            keyword in query_lower for keyword in youtube_keywords
        )
        ...
        if has_youtube_keyword:
            try:
                ...
                videos = await asyncio.to_thread(
                    self.search_videos, query=query, max_results=5
                )
                if videos:
                    logger.info(
                        f"YouTubeMCP: Fetched {len(videos)} videos for query: {query}"
                    )
