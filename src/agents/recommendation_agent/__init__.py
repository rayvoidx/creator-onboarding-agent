...
class RecommendationAgent(BaseAgent[RecommendationState]):
    ...
    def _calculate_recommendation_score(
        ...
        # 난이도 매칭 (40% 가중치)
        difficulty_match = self._get_difficulty_match_score(
            material["difficulty"], competency_level
        )
    ...
    def _get_difficulty_match_score(
        self, material_difficulty: str, user_level: str
    ) -> float:
        """난이도 매칭 점수"""
    ...
    def _get_interest_match_score(
        self, material_topics: List[str], user_interests: List[str]
    ) -> float:
        """관심사 매칭 점수"""
    ...
    def _calculate_confidence(
        ...
        # 사용자 프로필 완성도
        if user_profile:
            profile_completeness = len([v for v in user_profile.values() if v]) / max(
                len(user_profile), 1
            )
    ...
    def _generate_recommendation_reason(
        self, material: Dict[str, Any], competency_level: str, learning_style: str
    ) -> str:
        """추천 이유 생성"""
        reasons = []
