...
class CompetencyAgent(BaseAgent[CompetencyDiagnosisState]):
    ...
    async def execute(self, state: CompetencyDiagnosisState) -> CompetencyDiagnosisState:
        """역량진단 메인 실행 로직"""
        try:
    ...
    async def _preprocess_data(self, state: CompetencyDiagnosisState) -> CompetencyDiagnosisState:
        """데이터 전처리 및 보안 검증"""
        try:
            ...
            for r in state.responses:
                if isinstance(r, UserResponse):
                    ...
                    try:
                        ...
                        ans = r.get('answer') or ''
                        normalized_responses.append(UserResponse.model_validate({
            ...
            # 개인정보 익명화 처리
            anonymized_data = await self.security_tool.anonymize_personal_data(normalized_responses)
    ...
    async def _analyze_competency(self, state: CompetencyDiagnosisState) -> CompetencyDiagnosisState:
        """역량 분석 실행"""
        try:
            ...
            for q in state.questions:
                if isinstance(q, CompetencyQuestion):
                    ...
                    try:
                        normalized_questions.append(CompetencyQuestion.model_validate(q))
                    except Exception:
                        # 필수 필드 최소 보정
                        qid = q.get('question_id') or q.get('id') or ''
                        normalized_questions.append(CompetencyQuestion.model_validate({
                            'id': qid or 'q',
                            'question_id': qid or 'q',
                            'question_text': q.get('question_text') or q.get('text') or '',
                            'domain': q.get('domain') or 'education',
                            'competency_area': q.get('competency_area') or 'general',
                        }))
                else:
                    continue
            ...
            # 역량 영역별 분석
            competency_scores = await self._calculate_competency_scores(df, state.questions)
            
            # 강점/약점 분석
            strengths, weaknesses = await self._identify_strengths_weaknesses(competency_scores)
    ...
    async def _generate_recommendations(self, state: CompetencyDiagnosisState) -> CompetencyDiagnosisState:
        """개인별 맞춤형 추천 생성"""
        try:
            ...
            self.logger.info(f"Generated {len(prioritized_recommendations)} recommendations")
    ...
    async def _postprocess_results(self, state: CompetencyDiagnosisState) -> CompetencyDiagnosisState:
        """결과 후처리 및 보안 적용"""
        try:
            # 결과 데이터 암호화
            ...
            state.context["retention_policy"] = {
                ...
                "deletion_date": (datetime.now().replace(year=datetime.now().year + 1)).isoformat()
            }
    ...
    async def _calculate_competency_scores(
        ...
        for area in set(question_mapping.values()):
            area_questions = [qid for qid, area_name in question_mapping.items() if area_name == area]
            ...
            if not area_responses.empty:
                ...
                if weights.sum() > 0:
                    if ML_AVAILABLE and np is not None:
                        ...
                        weighted_score = sum(v * w for v, w in zip(values, weights_list)) / sum(weights_list)
                else:
    ...
    async def _identify_strengths_weaknesses(
        ...
        strengths = [area for area, score in competency_scores.items() if score >= threshold_high]
        weaknesses = [area for area, score in competency_scores.items() if score <= threshold_low]
    ...
    async def _analyze_learning_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        patterns['response_pattern'] = 'consistent' if patterns.get('confidence_consistency', 0) > 0.8 else 'variable'
        
        return patterns
    
    async def _calculate_overall_level(self, competency_scores: Dict[str, float]) -> str:
        """전체 역량 수준 계산"""
        if not competency_scores:
            return "미평가"
    ...
    async def _generate_weakness_recommendations(self, weaknesses: List[str]) -> List[Dict[str, Any]]:
        """약점 기반 추천 생성"""
        recommendations = []
    ...
    async def _generate_strength_recommendations(self, strengths: List[str]) -> List[Dict[str, Any]]:
        """강점 확장 추천 생성"""
        recommendations = []
