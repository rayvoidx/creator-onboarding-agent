...
class MainOrchestrator:
    ...
    def _with_agent_context(
        self, state: MainOrchestratorState, agent_key: str
    ) -> Dict[str, Any]:
        ...
        if agent_key == "mission":
            ...
            if creator_profile:
                ...
                if creator_profile.get("youtube_handle"):
                    youtube_cfg.setdefault(
                        "channel_handle", creator_profile["youtube_handle"]
                    )
                if creator_profile.get("creator_handle"):
                    youtube_cfg.setdefault(
                        "channel_handle", creator_profile["creator_handle"]
                    )
                keywords = creator_profile.get("keywords") or creator_profile.get(
                    "tags"
                )
                ...
                if isinstance(social_links, list):
                    scrape_candidates.extend(
                        [link for link in social_links if isinstance(link, str)]
                    )
                for key in (
                    "instagram_url",
                    "tiktok_url",
            ...
            supadata_filter = (
                filters.get("supadata") if isinstance(filters, dict) else {}
            )
            if supadata_filter:
                supadata_cfg = dict(spec.get("supadata", {}))
    ...
    def _setup_checkpointer(self) -> None:
        ...
        if not _CHECKPOINTER_AVAILABLE:
            self.logger.warning(
                "LangGraph SqliteSaver not available; proceeding without persistent checkpoints"
            )
    ...
    async def _route_request(
        self, state: MainOrchestratorState
        ...
        try:
            ...
            if self.deep_agents.should_use_deep_agents(message_content):
                ...
                if intent_str == UserIntent.COMPETENCY_ASSESSMENT.value:
                    ...
                    state.workflow_type = (
                        "rag" if self._should_use_rag(message_content) else "general"
                    )

            try:
                state.agent_model_config = self.settings.get_agent_config(
                    state.workflow_type
                )
            except Exception:
...
"""
            ...
            try:
                ...
                plan = {
                    ...
                    "needs_tools": state.workflow_type
                    in ("data_collection", "mission"),
    ...
    async def _tool_enrichment(
        ...
        try:
            ...
            try:
                if needs_tools and cost_pref != "budget":
                    got_supadata = isinstance(enrichment, dict) and bool(
                        enrichment.get("supadata")
                    )
                    if not got_supadata:
                        urls = []
                        if isinstance(enrichment, dict):
                            urls = (
                                (
                                    (enrichment.get("external_sources") or {}).get(
                                        "web"
                                    )
                                    or {}
                                ).get("urls")
                        ...
                        if isinstance(urls, list) and urls:
                            ...
                            sup_enrichment = await self.mcp_service.enrich_context(
                                sup_spec, ctx
                            )
            ...
            state.audit_trail.append(
                {
                    ...
                    "agent_key": agent_key,
                    "needs_tools": needs_tools,
                    "enriched_keys": state.tool_enrichment_result.get(
                        "enriched_keys", []
                    ),
                }
            )
    ...
    def _post_tool_enrichment_condition(self, state: MainOrchestratorState) -> str:
        ...
        try:
            ...
            te = (
                state.tool_enrichment_result
                if isinstance(state.tool_enrichment_result, dict)
                else {}
            )
...
"""
    ...
    async def _deep_agents_processing(
        self, state: MainOrchestratorState
    ) -> MainOrchestratorState:
        """Deep Agents 처리"""
        try:
            ...
            state.performance_metrics["deep_agents"] = {
                ...
                "processing_time": deep_agents_result.get("result", {})
                .get("metadata", {})
            ...
            self.logger.info(
                f"Deep Agents processing completed: {deep_agents_result.get('success', False)}"
            )
    ...
    def _deep_agents_route_condition(self, state: MainOrchestratorState) -> str:
        ...
        if not state.deep_agents_result or not state.deep_agents_result.get(
            "success", False
        ):
    ...
    async def _competency_diagnosis(
        self, state: MainOrchestratorState
    ) -> MainOrchestratorState:
        """역량진단 처리 (DER-001)"""
        try:
    ...
    async def _generate_recommendations(
        ...
        try:
            ...
            recommendation_result = await self.recommendation_agent.execute(
    ...
    async def _mission_recommendation(
        self, state: MainOrchestratorState
    ) -> MainOrchestratorState:
        ...
        try:
            ...
            mission_state = MissionRecommendationState(
                ...
                context=ctx,
                creator_id=str(
                    creator_profile.get("creator_id") or creator_profile.get("id") or ""
                ),
    ...
    async def _external_integration(
        self, state: MainOrchestratorState
    ) -> MainOrchestratorState:
        ...
        try:
            ...
            state.external_api_results = getattr(
                integration_result, "integration_results", {}
            )
    ...
    async def _analytics_processing(
        self, state: MainOrchestratorState
    ) -> MainOrchestratorState:
        ...
        try:
            ...
            if state.audit_trail and len(state.audit_trail) > 0:
                state.performance_metrics["total_execution_time"] = (
                    datetime.now()
                    - datetime.fromisoformat(state.audit_trail[0]["timestamp"])
                ).total_seconds()
            else:
    ...
    async def _synthesize_results(self, state: MainOrchestratorState) -> str:
        ...
        try:
            ...
            payload: Dict[str, Any] = {
                ...
                "missions": (
                    state.mission_recommendations[:5]
                ...
                "rag": {
                    ...
                    "answer": (
                        (state.rag_result or {}).get("response")
...
"""
            ...
            if (
                state.rag_result
                and isinstance(state.rag_result, dict)
                and isinstance(state.rag_result.get("response"), str)
            ):
    ...
    async def _data_collection(
        ...
        try:
            ...
            # 데이터 수집 에이전트 실행
            collection_state = await self.data_collection_agent.execute(
                collection_state
            )
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "mission_recommendations": getattr(
                    result, "mission_recommendations", []
                ),
            }
    ...
    async def _rag_processing(
        ...
        try:
            ...
            user_context = {
                ...
                "session_id": state.session_id,
                "competency_level": (
                    state.competency_data.get("level")
                    if state.competency_data
                    else None
                ),
            ...
            if rag_result.get("success", False):
                ...
                # 후속 워크플로우 결정
                state.workflow_type = self._determine_post_rag_workflow(
                    rag_result, str(message_content)
                )
                ...
                state.add_error(
                    f"RAG 처리 실패: {rag_result.get('error', 'Unknown error')}"
                )
    ...
    def _determine_query_type(self, message_content: str) -> PromptType:
        ...
        try:
            ...
            # 추천 관련
                ...
                keyword in message_lower
                for keyword in ["추천", "학습자료", "과정", "프로그램"]
            ):
                return PromptType.RECOMMENDATION

            # 검색 관련
            elif any(
                keyword in message_lower for keyword in ["검색", "찾기", "정보", "자료"]
            ):
    ...
    def _determine_post_rag_workflow(
        self, rag_result: Dict[str, Any], message_content: str
    ) -> str:
        """RAG 처리 후 워크플로우 결정"""
        try:
            # RAG 결과에 따른 후속 처리 결정
    ...
    def _rag_route_condition(self, state: MainOrchestratorState) -> str:
        ...
        try:
            ...
            # - alternate workflow
            if (
                self._should_replan_after_rag(state)
        ...
        try:
            ...
            if state and state.values:
                return {
                    ...
                    "last_updated": state.config.get("configurable", {}).get(
                        "thread_id", ""
                    ),
                    "state_exists": True,
                }
            else:
                return {"session_id": session_id, "state_exists": False}
    ...
    async def resume_session(self, session_id: str, new_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            updated_messages = list(current_state.values.get("messages", [])) + [
                new_message_obj
            ]
            ...
            updated_state = MainOrchestratorState(
                ...
                external_api_results=current_state.values.get(
        ...
        try:
            ...
            try:
                ...
                if callable(deleter):
                    ...
                    self.logger.info(
                        f"Cleared session via checkpointer API: {session_id}"
                    )
                    return True
            except Exception:
                # 아래 로우 SQL 경로로 폴백
            ...
            if conn is None:
                self.logger.info(
                    f"Checkpointer connection not available; marking session cleared: {session_id}"
                )
                return True
            ...
            try:
                ...
                pass
            self.logger.info(
                f"Session {session_id} cleared from checkpointer (rows: {deleted})"
            )
            return True
