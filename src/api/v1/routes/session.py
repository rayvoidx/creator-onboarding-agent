...
async def get_session_state(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="세션 상태 조회 중 오류가 발생했습니다.")
...
async def resume_session(session_id: str, message: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not result.get('success', False):
            raise HTTPException(status_code=400, detail=result.get('error', '세션 복원에 실패했습니다.'))
...
async def clear_session(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': success,
            'message': '세션이 삭제되었습니다.' if success else '세션 삭제에 실패했습니다.',
            'timestamp': datetime.now().isoformat()
        }
