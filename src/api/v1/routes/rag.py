...
async def rag_query(
    ...
    try:
        ...
        if not deps.rag_pipeline:
            raise HTTPException(status_code=500, detail="RAG 파이프라인이 초기화되지 않았습니다.")
        ...
        if result.get('success'):
            ...
            raise HTTPException(status_code=500, detail=result.get('error', 'RAG 처리 중 오류가 발생했습니다.'))

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"RAG query failed: {e}")
        raise HTTPException(status_code=500, detail="RAG 질의응답 처리 중 오류가 발생했습니다.")
...
async def rag_query_stream(
    ...
    try:
        ...
        async def event_generator():
            ...
            try:
                vector_results = await deps.rag_pipeline.retrieval_engine.vector_search(
                    query=query,
                    limit=deps.rag_pipeline.max_retrieval_docs,
                    filters={}
                )
                keyword_results: List[Dict[str, Any]] = []
                if getattr(deps.rag_pipeline, 'enable_hybrid_search', True):
                    keyword_results = await deps.rag_pipeline.retrieval_engine.keyword_search(
                        query=query,
                        limit=deps.rag_pipeline.max_retrieval_docs
                    )
                ...
                yield "event: docs\n" + "data: " + json.dumps({"documents": top_docs}, ensure_ascii=False) + "\n\n"
            except Exception as e:
                yield f"event: docs\ndata: {{\"error\": \"{str(e)}\"}}\n\n"
                top_docs = []
            ...
            try:
                context = {"retrieved_documents": top_docs, "user_context": {"user_id": user_id, "session_id": session_id}}
            ...
            try:
                ...
                if isinstance(settings.DEFAULT_LLM_MODEL, str) and settings.DEFAULT_LLM_MODEL.startswith('gpt-') and settings.OPENAI_API_KEY:
            ...
            if not used_stream:
                try:
                    ...
                    for i, ch in enumerate(chunks):
                        payload = sanitize_output({"text": (ch + ('.' if i < len(chunks) - 1 else ''))})
                        yield "event: answer\n" + "data: " + json.dumps(payload, ensure_ascii=False) + "\n\n"
                except Exception as e:
        ...
        raise HTTPException(status_code=500, detail="RAG SSE 처리 중 오류가 발생했습니다.")
...
async def get_rag_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="RAG 통계 조회 중 오류가 발생했습니다.")
