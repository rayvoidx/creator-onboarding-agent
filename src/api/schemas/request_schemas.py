...
class CompetencyAssessmentRequest(BaseModel):
    ...
    assessment_data: Dict[str, Any] = Field(
        default_factory=dict, description="진단 데이터"
    )
...
class RecommendationRequest(BaseModel):
    ...
    user_profile: Dict[str, Any] = Field(
        default_factory=dict, description="사용자 프로필"
    )
    learning_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="학습 선호도"
    )
    competency_data: Dict[str, Any] = Field(
        default_factory=dict, description="역량 데이터"
    )
...
class AnalyticsRequest(BaseModel):
    ...
    report_type: str = Field(
        ..., min_length=1, max_length=50, description="리포트 유형"
    )
    ...
    def validate_report_type(cls, v: str) -> str:
        ...
        if v.strip().lower() not in allowed_types:
            raise ValueError(
                f"리포트 유형은 다음 중 하나여야 합니다: {', '.join(allowed_types)}"
            )
...
class CreatorEvaluationRequest(BaseModel):
    ...
    handle: str = Field(
        ..., min_length=1, max_length=100, description="creator handle or id"
    )
    profile_url: Optional[str] = Field(None, description="public profile url")
    metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="optional known metrics (followers, posts_30d, avg_likes, brand_fit, reports_90d)",
    )
...
class MissionRecommendRequest(BaseModel):
    ...
    creator_id: str = Field(
        ..., min_length=1, max_length=100, description="크리에이터 ID"
    )
    creator_profile: Dict[str, Any] = Field(
        default_factory=dict,
        description="크리에이터 메트릭/프로필 (followers, engagement_rate 등)",
    )
